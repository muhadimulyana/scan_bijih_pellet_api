...
class BstController extends Controller
{
    ...
    public function store(Request $request)
    {
 	...
 	$this->validate($request, [
        ...
        foreach ($records as $rec) {
            ...
            array_push($barcodes, $rec['BARCODE']);
            array_push($kode_pellet, substr($rec['BARCODE'], 0, 13));

            if(!in_array(substr($rec['BARCODE'], 0, 13), $list_kode))
            {
        ...
        foreach($list_kode as $key => $value){
            ...
            $pellet = DB::table('erasystem_2012.pellet')->select('NAMA', 'NAMA_LABEL', 'KG')->where('KODE', $list_kode[$key])->first();
        ...
        $this->_setVariable('BST', $newpt, $pt_nama, $gudang, $dari_dept_id, $dari_dept_nama, $dari_dept_area, $NoTrans, $username, $status, $ket);
    ...
    public function checkBarcode(Request $request)
    {
        ...
        $result = DB::table('erasystem_2012.barcode_pellet')
            ...
            ->whereRaw('barcode_pellet_det.BARCODE = ? AND barcode_pellet_det.PT_ID = ? AND barcode_pellet_det.GUDANG = ? AND barcode_pellet_det.DEPT_ID = ? AND barcode_pellet_det.STATUS = ? AND barcode_pellet.AKTIF = ?', [$barcode, $newpt, $gudang, $dept, $newstatus, '1'])
    ...
    public function update(Request $request)
    {
        ...
        foreach ($records as $rec) {
            ...
            if($rec['STATUS_BARCODE'] == 1){
                ...
                //Handle untuk list kode
                if(!in_array(substr($rec['BARCODE'], 0, 13), $list_kode)) // Jika kode pellet belum ada di list dan statusnya = 1 maka masukkan kedalam list kode untuk barcode list pellet
                {
                    array_push($list_kode, substr($rec['BARCODE'], 0, 13));
                }
