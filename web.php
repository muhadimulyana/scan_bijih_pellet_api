...
$router->group(['prefix' => 'barcodePelletDet'], function () use ($router) {
    ...
    //NOTE: route untuk mengambil barcode berdasarkan notrans
    $router->get('/getBarcode/{notrans:[A-Za-z0-9/]+}', 'BarcodePelletDetController@getBarcode');
    //Note : route untuk mengambil list barcode dengan inner join ke barcode pellet det
...
$router->group(['prefix' => 'bst'], function () use ($router) {
    ...
    $router->post('/insert', 'BstController@store');
    $router->get('/getTotal/{notrans:[A-Za-z0-9/]+}', 'BstController@gettotalBarcode');
    $router->get('/cekList/{notrans:[A-Za-z0-9/]+}', 'BstController@checklistBst');
...
$router->group(['prefix' => 'doi'], function() use ($router){
    ...
    $router->get('/getlistdoiUser/{pt}/{gudang}', 'DoiController@getlistuserDoi'); //Sesuaikan
    ...
    $router->get('/getlistbarcodeUpdate/{IdDo:[A-Za-z0-9/]+}', 'DoiController@getlistbarcodeUpdate');
...
$router->group(['prefix' => 'exam'], function () use ($router) {
    $router->get('/test/{id:.*', 'ExampleController@testRoute');
    //$router->get('/deptScan', 'DepartemenController@getdeptScan');
});